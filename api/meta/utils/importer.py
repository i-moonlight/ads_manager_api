...
def update_or_create_ad(ad, user_id, access_token, account_id):
	...
	try:
		...
		print('effective_status is', effective_status)
		...
		if ads_platform is None:
			...
			# we should probably check to see if the image has already been processed, what if they change the creatives on the platform?
			if image_url is not None:
				process_image(ad_id, image_url, user_id)
